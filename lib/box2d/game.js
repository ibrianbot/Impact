...
class Box2DGame extends Game{
  ...
  loadLevel(data) {
    ...
    for (var i = 0; i < data.layer.length; i++) {
      ...
      if (ld.name == 'collision') {
        IG.instance.extensions.world = this.createWorldFromMap(ld.data, ld.width, ld.height, ld.tilesize)
        break
      }
    }
  ...
  update() {
    IG.instance.extensions.world.Step(
      IG.instance.system.tick,
      this.worldVelocityIterations,
      this.worldPositionIterations
    )
    IG.instance.extensions.world.ClearForces()
    super.update()
  }
  ...
  draw() {
    ...
    if (this.debugCollisionRects) {
      ...
      for (var i = 0; i < this.collisionRects.length; i++) {
        var rect = this.collisionRects[i]
        IG.instance.system.context.strokeStyle = '#00ff00'
        IG.instance.system.context.strokeRect(
          IG.instance.system.getDrawPos(rect.x * ts - this.screen.x),
          IG.instance.system.getDrawPos(rect.y * ts - this.screen.y),
          IG.instance.system.getDrawPos(rect.width * ts),
          IG.instance.system.getDrawPos(rect.height * ts)
        )
      }
    }
  }
