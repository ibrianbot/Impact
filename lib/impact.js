...
class IG {
	...
	static createDebugInstance(canvasId, gameClass, fps, width, height, scale, loaderClass){
		IG.debugActive = true
		IG.createInstance(canvasId, gameClass, fps, width, height, scale, loaderClass)
		createDebugger(IG.instance)
		return IG.instance
	}
	...
	static createInstance(canvasId, gameClass, fps, width, height, scale, loaderClass){
		...
		igConfig.input = new Input( this.system );
	...
	constructor(igConfig, gameClass, loaderClass){
		...
		var loader = null
		if (loaderClass){
			loader = new (loaderClass)(this.system, gameClass, this.resources)
		}else {
	...
	setAnimation( callback, element ) {
		...
		var animate = () => {
			if( !this.anims[current] ) { return; } // deleted?
