...
class Game {
	...
	static SORT = {
		Z_INDEX( a, b ){ return a.zIndex - b.zIndex; },
		POS_X( a, b ){ return (a.pos.x+a.size.x) - (b.pos.x+b.size.x); },
		POS_Y( a, b ){ return (a.pos.y+a.size.y) - (b.pos.y+b.size.y); }
	}
	...
	loadLevel( data ) {
		...
		for( var i = 0; i < data.layer.length; i++ ) {
			...
			if( ld.name == 'collision' ) {
				...
				var newMap = new BackgroundMap(ld.tilesize, ld.data, ld.tilesetName);
	...
	draw(){
		if( this.clearColor ) {
			IG.instance.system.clear( this.clearColor );
		}
		...
		this._rscreen.x = IG.instance.system.getDrawPos(this.screen.x)/IG.instance.system.scale;
		this._rscreen.y = IG.instance.system.getDrawPos(this.screen.y)/IG.instance.system.scale;
